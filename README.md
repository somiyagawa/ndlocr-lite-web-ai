<p align="center">
  <img src="https://img.shields.io/badge/version-4.4.0-teal?style=for-the-badge" alt="v4.4.0" />
...
**携帯版画像補正パネル超コンパクト化・タッチ操作修正** — パネルをmax-height 28vhに縮小し、ヘッダー・セクションラベルを非表示。スライダーを横一行レイアウトに変更（ラベル＋スライダー＋数値が一行で並ぶ）。スライダーにtouch-action: none＋stopPropagationを追加し、パネルスクロールとの干渉を解消。パネルを半透明化（backdrop-filter: blur）し、背面の画像を透かして確認可能に。

### v4.3.3 — 2026-03-27

**携帯版タッチ操作・ボトムシートUI・メニュー修正・アイコン改善** — ズームコントロール・オーバーレイ文字・信頼度・読み順ボタンがタッチに反応しなかった問題を修正。画像補正パネルをボトムシート方式に変更（画面下部からスライドアップし、画像を確認しながら補正可能）。保存メニューもボトムシート化（ツールバーoverflowによるクリップを解消）。メニューを左スライドに変更。冗長なUpload image/PDFボタン削除。フッター情報をメニューパネル下部に移動。文字オーバーレイボタンを「□字」、読み順ボタンを「①②」丸囲み数字アイコンに変更。
...
**オート自動判定・サンプルタイル選択** — オートモードに画像特徴分析（色温度・セピア度・紙の白さ）による現代/古典籍自動判定機能を実装。サンプル画像をビジュアルタイルで選択可能に（現代: 蜘蛛の糸、くずし字: 竹取物語）。RTMDetの入力サイズバグ（1280→1024）を修正。オートモード時に全モデルを並列プリロードし切替遅延を解消。
