    ...
    <encodingDesc>
      <appInfo>
        <application ident="ndlocr-lite-web-ai" version="3.7.0">
          <label>NDL(Kotenseki)OCR-lite Web AI Ultra Bluepond</label>
        </application>
      </appInfo>
    </encodingDesc>
${profileDescContent}
...
function buildPublicationStmt(metadata?: TEIMetadata): string {
  ...
  if (!metadata?.publisher && !metadata?.date) {
    lines.push(`        <p>Generated by NDL(Kotenseki)OCR-lite Web AI Ultra Bluepond</p>`)
  }
...
export function downloadTEI(result: OCRResult, metadata?: TEIMetadata): void {
  ...
  triggerBlobDownload(url, `${baseName}_ocr.xml`)
}
...
`
  const blob = new Blob([xml], { type: 'application/xml;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  triggerBlobDownload(url, 'ocr_all_pages.xml')
}
