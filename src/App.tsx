...
export default function App() {
  ...
  const handleClear = () => {
    if (sessionResults.length > 0) {
      const msg = t(lang, 'app.confirmDiscard')
          ...
          {getPageNumbers().map((p, i) =>
            p === '...' ? (
              ...
              <button
                ...
                title={draggable && p <= maxIndex ? t(lang, 'nav.dragToReorder') : ''}
              >
                {p + 1}
              </button>
            )
      ...
      <main className="main">
        ...
        {!hasResults && !isWorking && !isModelLoading && !hasPendingImages && (
          <section className="upload-section">
            <div className="landing-title">
              ...
              <p className="landing-title-desc">{t(lang, 'app.landingDesc')}</p>
            </div>
            ...
            />
            <span className="bluepond-credit">{t(lang, 'app.backgroundCredit')}</span>
          </section>
        )}
                ...
                <div className="image-with-preprocess">
                  <div className="image-with-preprocess-main">
                    <ImageViewer
                      ...
                      adjustLabel={t(lang, 'viewer.adjust')}
                      lang={lang}
                    />
                    <p className="region-select-hint">
                      {t(lang, 'ocr.regionSelectHint')}
                    </p>
                  </div>
                  {showPreprocessPanel && (
        ...
        {(isLoadingFiles || isModelLoading) && (
          <div className="processing-section">
            <div className="processing-bluepond-title">
              ...
              <span className="processing-bluepond-desc">{t(lang, 'app.processingDesc')}</span>
            </div>
              ...
              <div className="file-loading-status">
                ...
                <span className="file-loading-message">
                  {fileLoadingState.currentPage != null && fileLoadingState.totalPages != null
                    ? t(lang, 'progress.renderingFile', { fileName: fileLoadingState.fileName, current: String(fileLoadingState.currentPage), total: String(fileLoadingState.totalPages) })
                    : t(lang, 'progress.loadingFile', { fileName: fileLoadingState.fileName })}
                </span>
              </div>
            )}
            ...
            <div className="result-content">
              ...
              {isProcessing && (
                <div className="result-progress-inline">
                  <ProgressBar jobState={jobState} lang={lang} />
                  <button className="btn btn-stop" onClick={handleStopProcessing} title={t(lang, 'ocr.stop')}>
                ...
                {!isProcessing && !readingOrderEditMode && (
                  <>
                    <button
                      ...
                      title={t(lang, 'ocr.editReadingOrderTitle')}
                    >
                      {t(lang, 'ocr.editReadingOrderLabel')}
                    </button>
                    <button className="btn btn-secondary btn-new-file" onClick={handleClear}>
                      {t(lang, 'ocr.processNewFiles')}
                    </button>
                    <button className="btn btn-secondary btn-bug-report-inline" onClick={() => setShowBugReport(true)} title={t(lang, 'ocr.bugReportTitle')}>
                      ...
                      <span className="bug-report-label-desktop">{t(lang, 'ocr.bugReportLabel')}</span>
                    </button>
                  </>
                )}
              </div>
                          ...
                          <div className="image-with-preprocess-main">
                            ...
                            {preprocessedUrls[selectedResultIndex + 10000] && (
                              <div className="region-action-bar" style={{ flexDirection: 'column', gap: '6px', alignItems: 'center' }}>
                                ...
                                >
                                  {isReOCRing
                                    ? t(lang, 'ocr.reOcrInProgress')
                                    : t(lang, 'ocr.reOcrPage')
                                  }
                                </button>
                              </div>
                            )}
                            ...
                            <p className="region-select-hint">
                              {t(lang, 'ocr.regionSelectHintResult')}
                            </p>
                          </div>
                          {showPreprocessPanel && (
                            <Suspense fallback={null}>
        ...
        {sessionResults.length > 0 && (
          <button
            ...
            title={t(lang, 'ocr.uploadNewFiles')}
          >
